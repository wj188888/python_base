...
class MqttClient:
    ...
    def _on_message(self, client, userdata, msg):  # 从服务器接受到消息后回调此函数
        print("\n主题:" + str(msg.topic) + " 消息:" + str(msg.payload)) #auto_code()
...
if __name__ == '__main__':
    ...
    client.loop()
    # wp = psutil()#自己定义的一个类
    auto_json = {"username": "wangjie", "age": 19}
    auto_json = json.dumps(auto_json)
    while True:
        data_json=auto_json #方法返回一个包含CPU和进程信息的JSON字符串
